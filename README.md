...
> ⚠️ **정직하게**: **빌드는 CI 가 매번 확인합니다**(우분투 + Qt6). 하지만 **창을 띄워 본 사람은 아직 없습니다** — 창이 필요한 확인이라 CI 로는 대신할 수 없습니다. 처음 돌려 보실 분은 아래 터미널의 **프롬프트 인식 · `cd` · Stop(트리째 죽이기) · F5** 네 가지를 먼저 봐 주세요. 그 넷이 되면 리눅스 지원은 끝입니다.
...
| **정의로 가기** | 이름 위에서 F12 — `func`/`class`/`val`/매개변수/`bring ... up` 을 만든 자리로 |
| **쓴 곳 모두 찾기** | Shift+F12 — 그 파일 안에서 그 이름이 나오는 자리 전부 |
| **이름 바꾸기** | F2 — **만드는 자리가 딱 하나일 때만** 해 줍니다 (아래 참고) |

> **이름 바꾸기가 조심스러운 까닭**: 지금은 자리(scope)를 따지지 않고 이름으로만 찾습니다. 그래서 같은 이름을 만드는 곳이 둘 이상이면(함수 둘이 각각 `x` 라는 매개변수를 쓰는 것처럼) **거절합니다** — 엉뚱한 곳까지 바꾸느니 안 바꾸는 편이 낫습니다. 함수·클래스·전역처럼 만드는 자리가 하나인 이름은 그대로 바뀝니다.
...
**큰 자물쇠 하나**가 Lumi 코드를 돌리는 실을 언제나 하나로 지킵니다. 자리를 내주는 곳은 **운영체제를 기다리는 자리**입니다 — `sleep`, `shell`, 그리고 웹서버가 손님·자료·보내기를 기다릴 때 (`bring http` 의 손님 쪽은 `shell` 로 curl 을 부르니 여기 들어옵니다).
...
> 순환 수집기는 **저절로 돕니다** — 살아 있는 일감들의 지역 값까지 함께 훑으므로 `gc()` 를 손으로 부를 일도, 일감이 도는 동안 피할 일도 없습니다. 예제: [`examples/concurrency.lumi`](examples/concurrency.lumi)
...
찾아 주는 것은 아홉 가지입니다. 전부 **이 저장소에서 실제로 겪은 사고**를 본떠 골랐습니다.
...
| **인자 개수가 안 맞음** (`func f(a)` 를 `f(1, 2)` 로) | 그 줄에 닿아야만 나던 오류를 돌리기 전에 알려 줍니다 |
...
| `http.getjson` · `http.postjson` | JSON 머리말을 알아서 붙이고 답도 `["data"]` 로 풀어 줌 |
| `http.download(주소, 파일이름)` | 파일로 곧장 받기 (큰 파일도 OK) |
| `http.serve(포트, 처리함수[, 몇번])` | 작은 웹 서버 열기 |

**답의 모양**: `{ "ok": true, "code": 200, "body": "...", "headers": {...} }`

**규칙 두 가지 — 이것만 기억하면 됩니다**
- **`404`·`500` 은 오류가 아닙니다.** 서버가 제대로 답한 것이라 `["code"]` 로 옵니다. `ok` 는 200~299 일 때만 `true`.
- **주소를 못 찾거나 연결이 끊기면 `NetworkError` 를 냅니다.** `catch NetworkError e:` 로 잡으세요.

#### 작은 웹 서버
```
func 처리(요청):
    // 요청 = {"method":"GET", "path":"/인사", "query":{"이름":"루미"}, "headers":{...}, "body":"..."}
    if 요청["path"] == "/인사":
        return {"code": 200, "body": "안녕 " + 요청["query"]["이름"]}
    return {"code": 404, "body": "없는 길"}
